...
const funciones = {
    cambiarPagina: function (valor) {
        const fill = document.querySelector('div[id=fill]');
        fill.innerHTML = content[valor];
        window.history.replaceState({}, '', valor);
        this.paginaHandler(valor);
    },
    paginaHandler: function (valor) {
        let links = document.querySelectorAll('span.fake-link');
        links.forEach(link => {
            link.onclick = function () {
                paginaActual = funciones.cambiarPagina(link.getAttribute('id'));
            }
        })
        ...
        if ((valor == 'login' || valor == 'register') && user == null) { //agregamos los tokens a las paginas de login y registro
            ...
            if (errores) {
                ...
                if (Object.entries(old_values).length !== 0) {
                    //restauramos los valores viejos
            ...
            paginaActual = funciones.cambiarPagina('home');
        }
        if (valor == 'register') { //cuando estamos en la pagina register
            //esta funcion muestra una preview de la imagen de prefil en el registro y la valida
    ...
    manejoNavProductos: async function () {
        ...
        nav.querySelector('button').onclick = async function () {
    ...
    createCard: function (arrayProductos) {
        ...
        if (arrayProductos && productosContainer) {
            ...
            arrayProductos.forEach(producto => {
                ...
                cardButton.setAttribute('href', '/test/producto/' + producto.id);
...
window.onload = async function () {
    paginaActual = funciones.cambiarPagina(pagina);
}